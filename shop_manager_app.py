    ...
    if current_id is not None:
        try:
            ...
            for i, opcao in enumerate(opcoes):
                if opcao.startswith(f"ID {current_id} -"):
                    indice = i
                    break
        except Exception:
            pass
...
def mensagem_solicitacao_criada(
    numero, tipo, descricao, quantidade, unidade, centro_custo, prioridade, usuario
):
    qtd_txt = ""
...
def verificar_alerta_orcamento(
    ano, mes, centro_custo_id, tipo_orcamento="OPEX", numero_pedido=""
):
    resumo = resumo_orcamento_mes(ano, mes, centro_custo_id, tipo_orcamento)
    if resumo["valor_orcado"] <= 0:
        return
    if (
        resumo["percentual"] >= resumo["alerta_percentual"]
        or resumo["percentual"] >= 100
    ):
...
def tem_permissao_aprovar():
    return st.session_state.perfil in ["admin", "aprovador"]


def tem_permissao_excluir():
    return st.session_state.perfil in ["admin", "aprovador"]
...
def notificar(
    ...
    if enviar_anexos_telegram and anexos is not None and not anexos.empty:
        for anexo in anexos.itertuples():
            legenda_anexo = (
                f"📎 Orçamento/anexo do pedido {numero}: {anexo.nome_arquivo}"
            )
            try:
    ...
    if enviar_anexos_telegram:
        if anexos is None or anexos.empty:
            canais.append(
                "Telegram enviado sem anexos do pedido"
                if ok_telegram
                else "Telegram não enviado"
            )
        else:
            canais.append(
                "Telegram enviado com anexos do pedido"
                if ok_telegram
                else "Telegram enviado com falha em algum anexo"
            )
    else:
        canais.append(
            "Telegram enviado somente mensagem"
            if ok_telegram
            else "Telegram não enviado"
        )
...
def tela_login():
    st.title("🔐 Login - Gerenciador de Compras")
...
def tela_dashboard():
    ...
    if coluna_data:
        ...
        pedidos_mes = df[
            (df[coluna_data] >= inicio_mes) & (df[coluna_data] <= fim_mes)
        ].copy()
    else:
        pedidos_mes = df.copy()
    ...
    for col_valor in ["valor_total", "total"]:
        if col_valor in pedidos_mes.columns:
            total_valor = (
                pd.to_numeric(pedidos_mes[col_valor], errors="coerce").fillna(0).sum()
            )
            break
    ...
    c3.metric(
        ...
        (
            len(
                pedidos_mes[
                    pedidos_mes["status"].isin(["Aberto", "Pendente", "Solicitado"])
                ]
            )
            if "status" in pedidos_mes.columns
            else 0
        ),
    )
    c4.metric(
        "Recebidos",
        (
            len(pedidos_mes[pedidos_mes["status"].isin(["Recebido", "Finalizado"])])
            if "status" in pedidos_mes.columns
            else 0
        ),
    )
    ...
    with c6:
        ...
        if "prioridade" in pedidos_mes.columns and not pedidos_mes.empty:
            pr_df = (
                pedidos_mes.groupby("prioridade")
                .size()
                .reset_index(name="total")
                .sort_values("total", ascending=False)
            )
    ...
    if "tipo_orcamento" in pedidos_mes.columns and not pedidos_mes.empty:
        tipo_df = (
            pedidos_mes.groupby("tipo_orcamento")
            .size()
    ...
    if pedidos_mes.empty:
        ...
        st.dataframe(
            pedidos_mes[cols or pedidos_mes.columns.tolist()].tail(20),
...
def tela_novo_pedido():
    ...
    if st.session_state.perfil not in ["admin", "almoxarifado", "aprovador"]:
        st.warning(
            "Somente Administrador, Almoxarifado e Aprovador "
            "podem criar pedido de compra."
        )
        return

    produtos = carregar_produtos()
    fornecedores = carregar_fornecedores()
    solicitacoes = carregar_solicitacoes_abertas("MATERIAL")

    if solicitacoes.empty:
        st.warning(
            "Não existem solicitações de material abertas. "
            "Crie uma solicitação antes de gerar o pedido de compra."
        )
        return

    if produtos.empty:
        st.warning(
            "Cadastre produtos no módulo Produtos / Estoque " "antes de fazer pedidos."
        )
        return

    if fornecedores.empty:
        st.warning("Cadastre pelo menos um fornecedor.")
        return
    ...
    st.markdown("### Origem do pedido")

    solicitacao_map = {"Pedido sem solicitação vinculada": None}

    if not solicitacoes.empty:
        for _, sol in solicitacoes.iterrows():
            descricao_resumida = str(sol["descricao"] or "").strip()

            if len(descricao_resumida) > 80:
                descricao_resumida = descricao_resumida[:77] + "..."

            label = (
                f"{sol['numero']} | "
                f"{descricao_resumida} | "
                f"{sol.get('prioridade') or 'Normal'}"
            )
    ...
    if solicitacao_id:
        solicitacao_selecionada = solicitacoes[
            solicitacoes["id"].astype(int) == int(solicitacao_id)
        ].iloc[0]

        st.info(f"Solicitação vinculada: " f"{solicitacao_selecionada['numero']}")

        c1, c2, c3 = st.columns(3)

        c1.write(
            f"**Solicitado por:** "
            f"{solicitacao_selecionada.get('solicitado_por') or ''}"
        )

        c2.write(
            f"**Prioridade:** "
            f"{solicitacao_selecionada.get('prioridade') or 'Normal'}"
        )

        c3.write(
            f"**Centro de custo:** "
            f"{solicitacao_selecionada.get('centro_custo') or 'Não informado'}"
        )

        st.write(f"**Descrição:** " f"{solicitacao_selecionada.get('descricao') or ''}")

        if solicitacao_selecionada.get("quantidade"):
            st.write(
                f"**Quantidade solicitada:** "
                f"{solicitacao_selecionada.get('quantidade')} "
                f"{solicitacao_selecionada.get('unidade') or ''}"
            )
    ...
    # ADICIONAR PRODUTOS
    # =========================================================
    st.markdown("### Adicionar itens ao pedido")

    tab_produto, tab_avulso = st.tabs(
        [
            "📦 Produto do estoque",
            "🛒 Item avulso",
        ]
    ...
    with tab_produto:
        with st.form(
            ...
            opts = [
                f"{r.codigo} - {r.nome} ({r.unidade or 'UN'})"
            ...
            with c1:
                escolhido = st.selectbox(
                    "Produto",
                    opts,
                    key="produto_estoque_novo_pedido",
                )
            ...
            with c3:
                valor_produto = st.number_input(
                    ...
                    min_value=0.0,
                    step=1.0,
                    key="valor_produto_novo_pedido",
                )

            obs_produto = st.text_input(
                "Observação",
                key="obs_produto_novo_pedido",
            )
            ...
            if adicionar_produto:
                if qtd_produto <= 0:
                    ...
                    produto = produtos.iloc[opts.index(escolhido)]

                    st.session_state.itens_novo_pedido.append(
                        {
                            "tipo_item": "ESTOQUE",
                            ...
                            "valor_unitario": float(valor_produto),
                            "valor_total": (float(qtd_produto) * float(valor_produto)),
                            "observacao_item": str(obs_produto or "").strip(),
                        }
                    )

                    st.success("Produto do estoque adicionado.")
                    st.rerun()
        ...
        with st.form(
            ...
            with c1:
                descricao_avulsa = st.text_input(
                    "Descrição do item",
                    key="descricao_avulsa_novo_pedido",
                )

            with c2:
                unidade_avulsa = st.selectbox(
                    "Unidade",
                    [
                        "UN",
                        "PC",
                    ...
                    key="unidade_avulsa_novo_pedido",
                )

            c3, c4 = st.columns(2)

            with c3:
                qtd_avulsa = st.number_input(
                    "Quantidade",
                    min_value=0.0,
                    step=1.0,
                    key="qtd_avulsa_novo_pedido",
                )

            with c4:
                valor_avulso = st.number_input(
                    "Valor unitário",
                    min_value=0.0,
                    step=1.0,
                    key="valor_avulso_novo_pedido",
                )

            obs_avulso = st.text_input(
                "Observação",
                key="obs_avulso_novo_pedido",
            )

            adicionar_avulso = st.form_submit_button(
                "Adicionar item avulso ao pedido",
                use_container_width=True,
            )

            if adicionar_avulso:
                descricao_limpa = str(descricao_avulsa or "").strip()
                    ...
                    st.session_state.itens_novo_pedido.append(
                        {
                            ...
                            "descricao_avulsa": descricao_limpa,
                            "unidade": unidade_avulsa,
                            "unidade_avulsa": unidade_avulsa,
                            "quantidade": float(qtd_avulsa),
                            "valor_unitario": float(valor_avulso),
                            "valor_total": (float(qtd_avulsa) * float(valor_avulso)),
                            "observacao_item": str(obs_avulso or "").strip(),
                        }
                    )
    ...
    if st.session_state.itens_novo_pedido:
        df = pd.DataFrame(st.session_state.itens_novo_pedido)
        ex = df.copy()

        if "tipo_item" not in ex.columns:
            ex["tipo_item"] = "ESTOQUE"

        ex["tipo"] = ex["tipo_item"].apply(
            lambda valor: (
                "🛒 Item avulso"
                if str(valor).upper() == "AVULSO"
                else "📦 Produto do estoque"
            )
        )

        ex["codigo_exibicao"] = ex.apply(
            lambda row: (
                "—"
                if str(row.get("tipo_item", "")).upper() == "AVULSO"
                else str(row.get("codigo") or "")
            ),
            axis=1,
        )

        ex["descricao"] = ex.apply(
            lambda row: str(row.get("descricao_avulsa") or row.get("produto") or ""),
            axis=1,
        )

        ex["valor_unitario_exibicao"] = ex["valor_unitario"].apply(valor_moeda)

        ex["valor_total_exibicao"] = ex["valor_total"].apply(valor_moeda)
        ...
        st.dataframe(
            ex[
                [
                    ...
                    "unidade",
                    "quantidade",
                    "valor_unitario_exibicao",
                    "valor_total_exibicao",
                    "observacao_item",
                ]
            ].rename(
                columns={
                    "tipo": "Tipo",
                    "codigo_exibicao": "Código",
                    "descricao": "Descrição",
                    "unidade": "Unidade",
                    "quantidade": "Quantidade",
                    "valor_unitario_exibicao": "Valor unitário",
                    "valor_total_exibicao": "Valor total",
                    "observacao_item": "Observação",
                }
            ),
            use_container_width=True,
            hide_index=True,
        ...
        total = sum(
            ...
            for item in st.session_state.itens_novo_pedido
        )
        ...
        with c2:
            itens = [
                f"{i + 1} - {item['produto']}"
                for i, item in enumerate(st.session_state.itens_novo_pedido)
            ]
    ...
    total = sum(item["valor_total"] for item in st.session_state.itens_novo_pedido)
    st.metric("Total do Pedido", valor_moeda(total))
    ...
    centro_custo_solicitacao = None
    observacao_padrao = ""

    if solicitacao_selecionada is not None:
        prioridade_solicitacao = str(
            solicitacao_selecionada.get("prioridade") or "Normal"
        )

        if prioridade_solicitacao in [
            "Baixa",
            "Normal",
            "Alta",
            "Urgente",
        ]:
            prioridade_padrao = prioridade_solicitacao
        centro_valor = solicitacao_selecionada.get("centro_custo_id")
        if centro_valor is not None and not pd.isna(centro_valor):
            centro_custo_solicitacao = int(centro_valor)
        descricao_sol = str(solicitacao_selecionada.get("descricao") or "").strip()
        observacao_sol = str(solicitacao_selecionada.get("observacao") or "").strip()
        observacao_padrao = (
            f"Solicitação: "
            f"{solicitacao_selecionada.get('numero')}\n\n"
            f"Descrição solicitada:\n{descricao_sol}"
        )
    ...
    prioridade_index = prioridades.index(prioridade_padrao)
    ...
    with st.form("finalizar"):
        ...
        if centro_custo_solicitacao:
            centro_custo_id = centro_custo_solicitacao

            st.info(
                "Centro de custo herdado da solicitação: "
                f"{solicitacao_selecionada.get('centro_custo') or ''}"
            )
        else:
            centro_custo_id = seletor_centro_custo(
        ...
        st.caption(
            "O tipo OPEX/CAPEX será puxado automaticamente "
            "do orçamento mensal do centro de custo."
        )
        ...
        if salvar:
            if not st.session_state.itens_novo_pedido:
                st.error("Adicione pelo menos um produto ao pedido.")
                return

            fornecedor_id = int(
                fornecedores[fornecedores["nome"] == fornecedor_nome]["id"].iloc[0]
            )

            pedido_id, numero = criar_pedido(
                data_pedido=data,
                fornecedor_id=fornecedor_id,
                prioridade=prioridade,
                itens=st.session_state.itens_novo_pedido,
                observacao=obs,
            ...
            adir = Path("shop_data") / "anexos" / numero
            adir.mkdir(parents=True, exist_ok=True)
            ...
            total = sum(
                item["valor_total"] for item in st.session_state.itens_novo_pedido
            )

            pdf = gerar_pdf_pedido(pedido_id)

            msg = mensagem_pedido_criado(
                numero,
                pedido_id,
                st.session_state.usuario,
                fornecedor_nome,
            ...
            notificar(
                ...
                st.session_state.usuario,
                pdf,
                f"📄 Ordem de Compra {numero}",
                enviar_anexos_telegram=True,
                enviar_email=True,
            )
            ...
            if solicitacao_id:
                st.success(
                    f"Pedido {numero} criado e solicitação "
                    f"{solicitacao_selecionada['numero']} convertida."
                )
            else:
                st.success(f"Pedido {numero} criado.")
...
def tela_pedidos():
    ...
    sf = st.selectbox(
        "Filtrar por status",
        ["Todos", "Aberto", "Aprovado", "Comprado", "Recebido", "Cancelado"],
    )
    pf = st.selectbox(
        "Filtrar por prioridade", ["Todas", "Baixa", "Normal", "Alta", "Urgente"]
    )
    cc_filtro = st.selectbox(
        "Filtrar por centro de custo",
        (
            ["Todos"]
            + sorted([str(x) for x in pedidos["centro_custo"].dropna().unique()])
            if "centro_custo" in pedidos.columns
            else ["Todos"]
        ),
    )
    df = pedidos.copy()
    if sf != "Todos":
        df = df[df.status == sf]
    if pf != "Todas":
        df = df[df.prioridade == pf]
    if "centro_custo" in df.columns and cc_filtro != "Todos":
        df = df[df.centro_custo == cc_filtro]
    ex = df.copy()
    ex["status"] = ex.status.apply(lambda x: f"{emoji_status(x)} {x}")
    ex["prioridade"] = ex.prioridade.apply(lambda x: f"{emoji_prioridade(x)} {x}")
    ex["valor_total"] = ex.valor_total.apply(valor_moeda)
    st.dataframe(ex, use_container_width=True)
    st.divider()
    opts = [
        f'{r.numero} | {r.data} | {r.fornecedor or "Sem fornecedor"} | {r.status} | {emoji_prioridade(r.prioridade)} {r.prioridade}'
        for r in pedidos.itertuples()
    ]
    esc = st.selectbox("Pedido", opts)
    pedido_id = int(pedidos.iloc[opts.index(esc)].id)
    pedido = buscar_pedido(pedido_id)
    st.markdown(
        f"### Pedido {pedido['numero']} - {emoji_status(pedido['status'])} {pedido['status']} - {emoji_prioridade(pedido['prioridade'])} {pedido['prioridade']}"
    )
    ...
    if not itens.empty:
        it = itens.copy()
        it["valor_unitario"] = it.valor_unitario.apply(valor_moeda)
        it["valor_total"] = it.valor_total.apply(valor_moeda)
        st.dataframe(
            it[
                [
                    "codigo",
    ...
    st.markdown("### 📎 Anexos")
    anexos = carregar_anexos_pedido(pedido_id)
    if anexos.empty:
        st.info("Nenhum anexo.")
    else:
        for a in anexos.itertuples():
    ...
    novos = st.file_uploader(
        "Adicionar anexo ao pedido", accept_multiple_files=True, key=f"up_{pedido_id}"
    )
    if novos and st.button("Salvar anexos"):
        adir = Path("shop_data") / "anexos" / pedido["numero"]
        adir.mkdir(parents=True, exist_ok=True)
        for arq in novos:
            dest = adir / arq.name
            dest.write_bytes(arq.getbuffer())
            inserir_anexo(pedido_id, arq.name, str(dest), st.session_state.usuario)
        st.success("Anexos salvos.")
        st.rerun()
    st.divider()
    st.subheader("✅ Aprovação / Status")
    c1, c2, c3, c4 = st.columns(4)
    ...
    def mudar(ns, leg):
        ...
        notificar(
            ...
            pdf2,
            leg,
            enviar_email=enviar_email_status,
        )
    ...
    with c1:
        if tem_permissao_aprovar():
            if st.button("✅ Aprovar"):
                mudar("Aprovado", f"✅ Pedido {pedido['numero']} aprovado")
        else:
            st.info("Sem permissão para aprovar.")
    with c2:
        if st.button("🛒 Comprado"):
            mudar("Comprado", f"🛒 Pedido {pedido['numero']} comprado")
    with c3:
        if st.button("📦 Recebido"):
            mudar("Recebido", f"📦 Pedido {pedido['numero']} recebido")
    with c4:
        if st.button("🚫 Cancelado"):
            mudar("Cancelado", f"🚫 Pedido {pedido['numero']} cancelado")
    st.divider()
    ...
    with st.form("editar"):
        try:
            data_atual = datetime.strptime(str(pedido["data"]), "%Y-%m-%d").date()
        except Exception:
            data_atual = date.today()
        data = st.date_input("Data", value=data_atual)
        lista = fornecedores.nome.tolist()
        fn = ""
        if pedido["fornecedor_id"]:
            ...
            if not m.empty:
                fn = m.nome.iloc[0]
        fornecedor_nome = st.selectbox(
            "Fornecedor", lista, index=lista.index(fn) if fn in lista else 0
        )
        prioridade = st.selectbox(
            "Prioridade",
            ["Baixa", "Normal", "Alta", "Urgente"],
            index=["Baixa", "Normal", "Alta", "Urgente"].index(
                pedido["prioridade"] or "Normal"
            ),
        )
        obs = st.text_area(
            "Observação / Log", value=pedido["observacao"] or "", height=220
        )
        edit = []
        pops = [
            f'{r.codigo} - {r.nome} ({r.unidade or "UN"})'
        ...
        for idx, item in itens.iterrows():
            ...
            atual = f"{item['codigo']} - {item['produto']} ({item['unidade'] or 'UN'})"
            pi = pops.index(atual) if atual in pops else 0
            with a:
                pl = st.selectbox(
                    f"Produto item {idx+1}", pops, index=pi, key=f"p_{item.id}"
                )
            with b:
                qtd = st.number_input(
                    f"Quantidade item {idx+1}",
                    min_value=0.0,
                    step=1.0,
            ...
            obsi = st.text_input(
                f"Observação item {idx+1}",
                value=item.observacao_item or "",
                key=f"o_{item.id}",
            )
            pr = produtos.iloc[pops.index(pl)]
            edit.append(
                {
                    "produto_id": int(pr.id),
                    "quantidade": float(qtd),
                    "valor_unitario": float(val),
                    "observacao_item": obsi,
                }
            )
        if st.checkbox("Adicionar mais um item neste pedido"):
            pl = st.selectbox("Novo produto", pops)
            qtd = st.number_input("Quantidade novo item", min_value=0.0, step=1.0)
            val = st.number_input("Valor unitário novo item", min_value=0.0, step=1.0)
            obsi = st.text_input("Observação novo item")
            if qtd > 0:
                pr = produtos.iloc[pops.index(pl)]
                edit.append(
                    {
        ...
        if st.form_submit_button("Salvar Alterações do Pedido"):
            fid = int(fornecedores[fornecedores.nome == fornecedor_nome].id.iloc[0])
            valid = [i for i in edit if i["quantidade"] > 0]
            if not valid:
                st.error("O pedido precisa ter pelo menos um item.")
            else:
                atualizar_pedido(
                    pedido_id,
                    data,
                ...
                msg = mensagem_pedido_editado(
                    pedido["numero"], pedido_id, st.session_state.usuario, prioridade
                )
                notificar(
                    "Pedido editado",
                    pedido_id,
                    ...
                    st.session_state.usuario,
                    pdf2,
                    f"✏️ Pedido {pedido['numero']} editado",
                )
                st.success("Pedido atualizado.")
                st.rerun()
    if tem_permissao_excluir():
        if st.checkbox(
            "Confirmo que desejo excluir este pedido permanentemente."
        ) and st.button("Excluir Pedido"):
            num = pedido["numero"]
            excluir_pedido(pedido_id)
            msg = mensagem_pedido_excluido(num, st.session_state.usuario)
            notificar("Pedido excluído", pedido_id, num, msg, st.session_state.usuario)
            st.success("Pedido excluído.")
            st.rerun()


def tela_notificacoes():
    st.subheader("🔔 Painel de Notificações")
    df = carregar_notificacoes()
    if df.empty:
        st.info("Nenhuma notificação registrada.")
        return
    c1, c2 = st.columns(2)
    c1.metric("Total", len(df))
    c2.metric("Não lidas", len(df[df.lida == 0]))
...
def tela_produtos():
    ...
    st.info(
        "Os pedidos de compra usam diretamente os produtos cadastrados em Produtos / Estoque. Não existe mais cadastro separado de produtos em Compras."
    )
    produtos = carregar_produtos()
    if produtos.empty:
...
def tela_fornecedores():
    ...
    with st.form("forn"):
        ...
        if st.form_submit_button("Salvar Fornecedor"):
            if nome.strip():
                inserir_fornecedor(nome, contato, telefone)
    ...
    if not fornecedores.empty:
        ...
        opts = [f"{r.id} - {r.nome}" for r in fornecedores.itertuples()]
        esc = st.selectbox("Selecione o fornecedor", opts)
        f = fornecedores.iloc[opts.index(esc)]
        with st.form("edit_forn"):
            n = st.text_input("Nome", value=f.nome)
            c = st.text_input("Contato", value=f.contato or "")
            t = st.text_input("Telefone", value=f.telefone or "")
            if st.form_submit_button("Salvar Alterações"):
                atualizar_fornecedor(int(f.id), n, c, t)
                st.success("Fornecedor atualizado.")
                st.rerun()
        if tem_permissao_excluir() and st.checkbox(
            "Confirmo que desejo excluir este fornecedor."
        ):
            if st.button("Excluir Fornecedor"):
                excluir_fornecedor(int(f.id))
...
def tela_configuracoes():
    ...
    st.markdown("### Logo do PDF")
    logo = st.file_uploader("Enviar logo PNG", type=["png"])
    if logo and st.button("Salvar logo"):
        Path("assets").mkdir(exist_ok=True)
        (Path("assets") / "logo.png").write_bytes(logo.getbuffer())
        st.success("Logo salva.")
    st.markdown("### E-mail SMTP")
    st.write(
        "Status:", "✅ Configurado" if email_configurado() else "⚠️ Não configurado"
    )
    st.caption(
        "Configure via Secrets ou .env: EMAIL_SMTP_HOST, EMAIL_SMTP_PORT, EMAIL_SMTP_USER, EMAIL_SMTP_PASSWORD, EMAIL_REMETENTE e EMAIL_DESTINATARIOS."
    )
    if st.button("Enviar e-mail de teste"):
        ok = enviar_email_notificacao(
            "Teste StockPro Compras", "E-mail de teste enviado pelo StockPro Compras."
        )
        (
            st.success("E-mail de teste enviado.")
            if ok
            else st.error("Falha no envio. Verifique Secrets, .env e SMTP.")
        )

    st.markdown("### Telegram")
    st.write(
        "Status:", "✅ Configurado" if telegram_configurado() else "⚠️ Não configurado"
    )
    st.caption(
        "Configure via Secrets ou .env: TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID e TELEGRAM_ATIVO."
    )
    if st.button("Enviar Telegram de teste"):
        ok = enviar_telegram(
            "🔔 Teste do StockPro Compras\n\nTelegram configurado com sucesso."
        )
        (
            st.success("Telegram de teste enviado.")
            if ok
            else st.error(
                "Falha no envio. Verifique token, chat_id e se o bot está no grupo."
            )
        )
...
def tela_centros_custo_orcamento():
    ...
    with t1:
        ...
        with st.form("novo_centro_custo", clear_on_submit=True):
            ...
            if st.form_submit_button(
                "Salvar centro de custo", use_container_width=True
            ):
                if not nome.strip():
        ...
        if centros.empty:
            ...
            selecionado = st.selectbox(
                "Selecionar centro de custo", list(centro_map.keys())
            )
            ...
            with st.form("editar_centro_custo"):
                ...
                with b2:
                    inativar = st.form_submit_button(
                        "Inativar", use_container_width=True
                    )
                with b3:
                    ...
                    if not nome_edit.strip():
                        ...
                        try:
                            atualizar_centro_custo(
                                int(row.id), nome_edit, descricao_edit, ativo_edit
                            )
            ...
            visual = centros.copy()
            visual["ativo"] = visual["ativo"].apply(
                lambda x: "Sim" if int(x) == 1 else "Não"
            )
            st.dataframe(
                visual.rename(
                    columns={
    ...
    with t2:
        ...
        st.info(
            "Cadastre primeiro o valor total mensal de OPEX e/ou CAPEX. Depois distribua esse valor entre os centros de custo."
        )
        ...
        with st.form("orcamento_geral_mensal"):
            ...
            mes = c2.number_input(
                "Mês", min_value=1, max_value=12, value=hoje.month, step=1, key="og_mes"
            )
            tipo_orcamento = c3.selectbox(
                "Tipo de orçamento", ["OPEX", "CAPEX"], key="og_tipo"
            )
            ...
            if st.form_submit_button(
                "Salvar orçamento geral", use_container_width=True
            ):
                try:
                    salvar_orcamento_geral(
                        ano, mes, tipo_orcamento, valor_total, alerta, observacao
                    )
        ...
        if orcs.empty:
            ...
            escolha_orc = st.selectbox(
                "Editar / excluir orçamento geral", list(mapa.keys()), key="og_edit_sel"
            )
            row = mapa[escolha_orc]

            with st.form("editar_orcamento_geral"):
                c1, c2, c3, c4 = st.columns(4)
                ano_e = c1.number_input(
                    "Ano",
                    min_value=2024,
                    max_value=2100,
                    value=int(row.ano),
                ...
                tipo_e = c3.selectbox(
                    "Tipo",
                    ["OPEX", "CAPEX"],
                    index=0 if row.tipo_orcamento == "OPEX" else 1,
                    key="og_e_tipo",
                )
                valor_e = c4.number_input(
                    "Valor total mensal",
                    min_value=0.0,
                ...
                alerta_e = st.slider(
                    ...
                    min_value=10,
                    max_value=100,
                    value=int(float(row.alerta_percentual)),
                    key="og_e_alerta",
                )
                obs_e = st.text_area(
                    "Observação",
                    value="" if pd.isna(row.observacao) else str(row.observacao),
                    key="og_e_obs",
                )
                ...
                with b1:
                    salvar = st.form_submit_button(
                        "Atualizar orçamento geral", use_container_width=True
                    )
                with b2:
                    excluir = st.form_submit_button(
                        "Excluir orçamento geral", use_container_width=True
                    )

                if salvar:
                    salvar_orcamento_geral(
                        ano_e, mes_e, tipo_e, valor_e, alerta_e, obs_e
                    )
        ...
        for tipo in ["OPEX", "CAPEX"]:
            with c1 if tipo == "OPEX" else c2:
                ...
                st.metric(
                    "Saldo para distribuir", valor_moeda(resumo["saldo_distribuir"])
                )
    ...
    with t3:
        ...
        st.info(
            "Defina aqui se o orçamento é OPEX ou CAPEX. O pedido herdará automaticamente esse tipo pelo centro de custo e mês."
        )
        hoje = datetime.now()
        centros = carregar_centros_custo()
        with st.form("orcamento_mensal"):
            c1, c2, c3, c4 = st.columns(4)
            ano = c1.number_input(
                "Ano", min_value=2024, max_value=2100, value=hoje.year, step=1
            )
            mes = c2.number_input(
                "Mês", min_value=1, max_value=12, value=hoje.month, step=1
            )
            tipo_orcamento = st.selectbox(
                "Tipo de orçamento",
                ["OPEX", "CAPEX"],
                key="tipo_orcamento_orcamento_mensal",
            )
            resumo_geral = resumo_orcamento_geral_mes(ano, mes, tipo_orcamento)
            st.caption(
                f"Orçamento geral {tipo_orcamento}: {valor_moeda(resumo_geral['valor_total'])} | Distribuído: {valor_moeda(resumo_geral['distribuido'])} | Saldo a distribuir: {valor_moeda(resumo_geral['saldo_distribuir'])}"
            )
            opcoes = ["Orçamento geral"] + [
                f"ID {int(r.id)} - {r.nome}" for r in centros.itertuples()
            ]
            escolha = c3.selectbox("Centro de custo", opcoes)
            cc_id = (
                None
                if escolha == "Orçamento geral"
                else int(escolha.split(" - ")[0].replace("ID ", ""))
            )
            valor = c4.number_input(
                "Valor orçado", min_value=0.0, value=0.0, step=100.0
            )
            alerta = st.slider(
                "Avisar quando atingir (%)", min_value=10, max_value=100, value=80
            )
            ...
            if st.form_submit_button("Salvar orçamento", use_container_width=True):
                try:
                    resumo_geral = resumo_orcamento_geral_mes(ano, mes, tipo_orcamento)
                    if (
                        resumo_geral["valor_total"] > 0
                        and valor > resumo_geral["saldo_distribuir"] + 0.01
                    ):
                        st.warning(
                            "O valor informado é maior que o saldo disponível para distribuir neste tipo de orçamento."
                        )
                    salvar_orcamento_mensal(
                        ano, mes, valor, alerta, cc_id, tipo_orcamento
                    )
    ...
    with t4:
        ...
        tipo_orcamento_consulta = st.selectbox(
            "Tipo de orçamento para consulta",
            ["OPEX", "CAPEX"],
            key="tipo_orc_consulta",
        )
        ...
        resumo = resumo_orcamento_mes(
            hoje.year, hoje.month, cc_id, tipo_orcamento_consulta
        )
...
def mensagem_servico_criado(
    numero, usuario, fornecedor, valor, prioridade, centro_custo, tipo_orcamento
):
...
def notificar_servico(
            ...
            for anexo in anexos.itertuples():
                try:
                    ok_doc = enviar_telegram_documento(
                        anexo.caminho,
                        f"📎 Anexo do serviço {numero}: {anexo.nome_arquivo}",
                    )
                    if not ok_doc:
                        ok_anexos = False
                except Exception as erro_doc:
    ...
    status_envio.append(
        "Telegram enviado"
        if ok_msg_telegram and ok_anexos
        else "Telegram não enviado ou anexo com falha"
    )
    registrar_notificacao(
        tipo, servico_id, numero, msg, " | ".join(status_envio), usuario
    )
...
def verificar_alerta_orcamento_servico(servico_id, numero_servico=""):
    ...
    resumo = resumo_orcamento_mes(
        data_ref.year, data_ref.month, centro_custo_id, tipo_orcamento
    )
...
def tela_servicos_dashboard():
    ...
    mes_df = df[
        (df["data"].dt.year == hoje.year) & (df["data"].dt.month == hoje.month)
    ].copy()

    st.caption(f"Indicadores do mês vigente: {hoje.strftime('%m/%Y')}")

    total_mes = (
        pd.to_numeric(mes_df["valor_total"], errors="coerce").fillna(0).sum()
        if not mes_df.empty
        else 0
    )
    c1, c2, c3, c4 = st.columns(4)
    ...
    c3.metric(
        "Abertos",
        (
            len(mes_df[mes_df["status"].isin(["Aberto", "Aprovado", "Executado"])])
            if not mes_df.empty
            else 0
        ),
    )
    c4.metric(
        "Finalizados",
        len(mes_df[mes_df["status"].isin(["Finalizado"])]) if not mes_df.empty else 0,
    )
    ...
    with c5:
        ...
        if not mes_df.empty:
            st.dataframe(
                mes_df.groupby("status").size().reset_index(name="total"),
                use_container_width=True,
                hide_index=True,
            )
        else:
    ...
    with c6:
        ...
        if not mes_df.empty and "tipo_orcamento" in mes_df.columns:
            tipo_df = (
                mes_df.groupby("tipo_orcamento")["valor_total"].sum().reset_index()
            )
...
def tela_novo_servico():
    ...
    fornecedores = carregar_fornecedores()
    solicitacoes = carregar_solicitacoes_abertas("SERVICO")

    if fornecedores.empty:
        st.warning("Cadastre pelo menos um fornecedor antes de criar serviços.")
        return

    st.markdown("### Origem do serviço")

    solicitacao_map = {}

    if not solicitacoes.empty:
        for _, sol in solicitacoes.iterrows():
            descricao_resumida = str(sol.get("descricao") or "").strip()
    ...
    if solicitacao_selecionada is not None:
        ...
        descricao_padrao = str(solicitacao_selecionada.get("descricao") or "").strip()

        observacao_solicitacao = str(
            solicitacao_selecionada.get("observacao") or ""
        ).strip()

        observacao_padrao = f"Solicitação: " f"{solicitacao_selecionada.get('numero')}"

        if observacao_solicitacao:
            observacao_padrao += (
                f"\n\nObservação da solicitação:\n" f"{observacao_solicitacao}"
            )
    ...
    with st.form("novo_servico"):
        ...
        data_servico = c1.date_input(
            "Data do serviço",
            value=date.today(),
        )

        fornecedor_nome = c2.selectbox(
            "Fornecedor",
            fornecedores["nome"].tolist(),
        )
        ...
        if salvar:
            ...
            if solicitacao_id:
                st.success(
                    f"Serviço {numero} cadastrado e solicitação "
                    f"{solicitacao_selecionada['numero']} convertida."
                )
            else:
...
def tela_servicos():
    ...
    status_f = filtros[0].selectbox(
        "Status", ["Todos"] + sorted(servicos_view["status"].dropna().unique().tolist())
    )
    tipo_f = filtros[1].selectbox(
        "Tipo",
        ["Todos"] + sorted(servicos_view["tipo_orcamento"].dropna().unique().tolist()),
    )
    cc_f = filtros[2].selectbox(
        "Centro de custo",
        ["Todos"] + sorted(servicos_view["centro_custo"].dropna().unique().tolist()),
    )
    ...
    mapa = {
        f"ID {int(r.id)} - {r.numero} - {r.fornecedor} - {valor_moeda(float(r.valor_total))}": int(
            r.id
        )
        for r in df.itertuples()
    }
    ...
    st.write(
        f"**Centro de custo:** {serv.get('centro_custo') or 'Sem centro de custo'}"
    )
    ...
    if st.session_state.perfil in [
        "admin",
        "almoxarifado",
        "aprovador",
    ]:
        with st.expander(
            "✏️ Editar dados do serviço",
            expanded=False,
        ):
            fornecedores = carregar_fornecedores()
            ...
            if fornecedores.empty:
                st.warning(
                    "Cadastre pelo menos um fornecedor " "antes de editar o serviço."
                )
            else:
                fornecedor_ids = fornecedores["id"].astype(int).tolist()

                fornecedor_atual_id = serv.get("fornecedor_id")

                try:
                    fornecedor_atual_id = int(fornecedor_atual_id)
                except Exception:
                    fornecedor_atual_id = fornecedor_ids[0]

                if fornecedor_atual_id in fornecedor_ids:
                    fornecedor_index = fornecedor_ids.index(fornecedor_atual_id)
                else:
                    fornecedor_index = 0
                ...
                prioridade_atual = str(serv.get("prioridade") or "Normal")
                ...
                with st.form(f"editar_servico_{sid}"):
                    c1, c2, c3 = st.columns(3)

                    data_edicao = c1.date_input(
                        "Data do serviço",
                        value=data_atual,
                        key=f"edit_serv_data_{sid}",
                    )

                    fornecedor_edicao = c2.selectbox(
                        "Fornecedor",
                        fornecedores["nome"].tolist(),
                        index=fornecedor_index,
                        key=f"edit_serv_fornecedor_{sid}",
                    )

                    prioridade_edicao = c3.selectbox(
                        "Prioridade",
                        prioridades,
                        index=prioridade_index,
                        key=f"edit_serv_prioridade_{sid}",
                    )

                    centro_custo_edicao = seletor_centro_custo(
                        "Centro de custo",
                        current_id=serv.get("centro_custo_id"),
                        key=f"edit_serv_cc_{sid}",
                    )

                    descricao_edicao = st.text_area(
                        "Descrição do serviço",
                        value=str(serv.get("descricao") or ""),
                        key=f"edit_serv_descricao_{sid}",
                    )

                    valor_edicao = st.number_input(
                        "Valor do serviço",
                        min_value=0.0,
                        value=float(serv.get("valor_total") or 0),
                        step=100.0,
                        key=f"edit_serv_valor_{sid}",
                    )

                    observacao_edicao = st.text_area(
                        "Observação",
                        value=str(serv.get("observacao") or ""),
                        key=f"edit_serv_observacao_{sid}",
                    )
                ...
                if salvar_edicao:
                    if not descricao_edicao.strip():
                        ...
                        fornecedor_id_edicao = int(
                            fornecedores[fornecedores["nome"] == fornecedor_edicao][
                                "id"
                            ].iloc[0]
                        )

                        tipo_orcamento = atualizar_servico(
                            servico_id=sid,
                            data_servico=data_edicao,
                            fornecedor_id=fornecedor_id_edicao,
                            centro_custo_id=centro_custo_edicao,
                            descricao=descricao_edicao,
                            valor_total=valor_edicao,
                            prioridade=prioridade_edicao,
                            observacao=observacao_edicao,
                            usuario=st.session_state.usuario,
                        )

                        st.success(
                            f"Serviço {serv['numero']} "
                            f"atualizado com sucesso. "
                            f"Orçamento: {tipo_orcamento}."
                        )
    ...
    def mudar(ns, legenda):
        ...
        msg = mensagem_status_servico(
            serv["numero"], ns, st.session_state.usuario, serv["prioridade"]
        )
        notificar_servico(
            f"Status {ns}",
            sid,
...
def tela_servicos_integrada():
    ...
    st.caption(
        "Controle de serviços usando fornecedores, centro de custo, orçamento e notificações do sistema."
    )
...
def main():
    ...
    )
    {
        ...
        "Solicitações": tela_solicitacoes,
        "Novo Pedido": tela_novo_pedido,
        "Pedidos": tela_pedidos,
        "Notificações": tela_notificacoes,
        "Produtos": tela_produtos,
        "Fornecedores": tela_fornecedores,
        "Centro de custo / Orçamento": tela_centros_custo_orcamento,
